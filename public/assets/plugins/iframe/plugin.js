  ...
  var iframe = (function() {
    ...
    tinymce.PluginManager.add("iframe", function(editor, url) {
      ...
      editor.ui.registry.addIcon(
        "frame",
        '<svg width="24" height="24"><use xlink:href="custom-icons.svg#file-text"></use></svg>'
      );
        ...
        onAction: function(instance, trigger) {
          ...
          setTimeout(() => {
            ...
            editor.insertContent(
              '<p>You clicked Dialog Button "' + trigger.name + '".</p>'
            );
            ...
            case "sayName":
              if (data.data.name == "") {
                ...
                editor.windowManager.alert(
                  "Hi there " + data.data.name + " - nice to meet you!"
                );
      ...
      editor.addCommand("iframeCommand", function(ui, value) {
        if (value == "") {
          ...
          editor.windowManager.alert(
            'You need type some text in the "iframe input".'
          );
        } else {
          // simply show an alert with the value being passed
          editor.windowManager.alert(
            'Hi, this is the iframeCommand from the iframe plugin. You sent me: "' +
              value +
              '"'
          );
          ...
          // insert some content
          editor.insertContent(
            '<p>Your "iframe input" was "' + value + '"</p>'
          );
        }
      });
      ...
      return {
        getMetadata: function() {
          return {
            ...
            url:
              "https://www.tiny.cloud/docs/enterprise/mentions/#gettingstartedwithtinymcementions"
          };
        }
      };
    });
